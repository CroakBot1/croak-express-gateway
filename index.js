...
const client = restClientV5({ // ✅ CORRECT
...
app.get('/fetch-balance', async (req, res) => {
  try {
    ...
    console.error('❌ Error fetching balance:', err?.response?.data || err.message || err);
...
app.get('/fetch-positions', async (req, res) => {
  try {
    ...
    console.error('❌ Error fetching positions:', err?.response?.data || err.message || err);
